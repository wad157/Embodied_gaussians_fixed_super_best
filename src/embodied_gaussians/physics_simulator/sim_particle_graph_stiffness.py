...
class OnlineTissueStiffnessSettings:
    ...
    # SUPER Future consumes a causal stride-2 AllTracker stream.  SIM uses
    # stride 1; making the observed stride explicit preserves the same five
    # consecutive-transition window without joining across withheld frames.
    causal_frame_stride: int = 1
    ...
    @property
    def graph_smoothing_iterations(self) -> int:
        """Compatibility name used by the current SUPER run manifest."""
        return self.spatial_smoothing_iterations

    @property
    def graph_smoothing_blend(self) -> float:
        """Compatibility name used by the current SUPER run manifest."""
        return self.spatial_smoothing_blend
    ...
    def validate(self) -> None:
        ...
        if self.causal_frame_stride < 1:
            raise ValueError("Causal frame stride must be positive")
...
class ResidualDrivenPaperStiffnessUpdater:
    ...
    def _append_causal_observation(
        ...
        *,
        ...
        if (
            history
            and frame_index
            != history[-1].frame_index + self.settings.causal_frame_stride
    ...
    def _propose_differentiable_local_relative(
        ...
        *,
        ...
        with torch.no_grad():
            # SIM's known grasp owns one immutable control patch, so the
            # original implementation accumulated exclusions forever. SUPER's
            # instrument/contact patch is dynamic. Keep only the current
            # observation excluded; otherwise a particle touched once can
            # never contribute material evidence again.
            self.local_parameter_exclusion_mask.copy_(
    ...
    def replace_candidate_step_with_particle_graph_lm(
        ...
        effective_step = candidate_coefficient - coefficient
        # Clipping before the zero-mean graph projection is insufficient:
        # subtracting the mass-weighted mean can amplify a nominal 0.02 step
        # (0.062 and 0.108 were observed in SUPER). Enforce the trust radius
        # on the final, physically installed coefficient displacement.
        effective_maximum = effective_step.abs().max()
        if float(effective_maximum.item()) > settings.maximum_log_step:
            effective_step = effective_step * (
                settings.maximum_log_step / effective_maximum
            )
            candidate_coefficient = coefficient + effective_step
    ...
    @property
    def distance_update_count(self) -> int:
        """Every accepted GraphLM transaction updates the distance field."""
        return self.update_count

    @property
    def shape_update_count(self) -> int:
        """GraphLM applies the same commits through its weak shape coupling."""
        return self.update_count
