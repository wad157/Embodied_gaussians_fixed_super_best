...
class VisualTissueResidualMappingSettings:
    ...
    # Profile out a bounded per-camera/channel affine exposure transform before
    # evaluating the geometric residual.  The nuisance parameters are detached:
    # gradients still move geometry, but cannot use exposure itself as a state.
    photometric_affine_calibration: bool = False
    photometric_gain_minimum: float = 0.5
    photometric_gain_maximum: float = 2.0
    photometric_bias_limit: float = 0.25
    ...
    previous_residual_carry: float = 0.0
    ...
    def validate(self) -> None:
        ...
        if self.photometric_gain_minimum <= 0.0:
            raise ValueError("Photometric gain minimum must be positive")
        if self.photometric_gain_maximum < self.photometric_gain_minimum:
            raise ValueError(
                "Photometric gain maximum must not be below its minimum"
            )
        if self.photometric_bias_limit < 0.0:
            raise ValueError("Photometric bias limit must be non-negative")
        weights = (
            self.data_weight,
        ...
        if not 0.0 <= self.previous_residual_carry <= 1.0:
            raise ValueError("Previous residual carry must lie in [0, 1]")
...
class VisualTissueResidualMappingResult:
    ...
    final_trajectory_hold_loss: float
    trajectory_hold_rms_m: float
    trajectory_hold_maximum_m: float
    trajectory_hold_track_count: int
...
def equal_camera_visual_loss(
    ...
    *,
    ...
    affine_color_calibration: bool = False,
    affine_gain_minimum: float = 0.5,
    affine_gain_maximum: float = 2.0,
    affine_bias_limit: float = 0.25,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute independently normalized camera losses and their equal mean.

    When requested, a bounded RGB affine transform is profiled independently
    for every camera and channel.  This removes exposure/white-balance error
    from the geometric observation without adding appearance state to the
    physical solver.  The fitted gain and bias are detached so optimization
    gradients can only change rendered geometry.
    """
    ...
    if affine_color_calibration:
        if affine_gain_minimum <= 0.0:
            raise ValueError("Affine color gain minimum must be positive")
        if affine_gain_maximum < affine_gain_minimum:
            raise ValueError("Affine color gain bounds are reversed")
        if affine_bias_limit < 0.0:
            raise ValueError("Affine color bias limit must be non-negative")
        channel_weights = pixel_weights[..., None]
        channel_weight_sums = channel_weights.sum(dim=(1, 2)).clamp_min(1.0e-8)
        rendered_mean = (
            rendered_colors * channel_weights
        ).sum(dim=(1, 2)) / channel_weight_sums
        target_mean = (
            target_colors * channel_weights
        ).sum(dim=(1, 2)) / channel_weight_sums
        rendered_centered = rendered_colors - rendered_mean[:, None, None, :]
        target_centered = target_colors - target_mean[:, None, None, :]
        covariance = (
            channel_weights * rendered_centered * target_centered
        ).sum(dim=(1, 2)) / channel_weight_sums
        rendered_variance = (
            channel_weights * rendered_centered.square()
        ).sum(dim=(1, 2)) / channel_weight_sums
        affine_gain = (covariance / rendered_variance.clamp_min(1.0e-6)).clamp(
            min=affine_gain_minimum,
            max=affine_gain_maximum,
        ).detach()
        affine_bias = (
            target_mean - affine_gain * rendered_mean
        ).clamp(
            min=-affine_bias_limit,
            max=affine_bias_limit,
        ).detach()
        rendered_colors = (
            rendered_colors * affine_gain[:, None, None, :]
            + affine_bias[:, None, None, :]
        )
...
class TetrahedralGaussianVisualResidualMapper:
    ...
    def _constraint_losses(
        ...
        *,
        ...
        """Evaluate geometry, using either rest or the pre-update state.

        A runtime residual is an incremental state-estimation correction.  Its
        regularizers must therefore be zero when the increment is zero, even
        if contact has already deformed the tissue away from rest.  Pulling
        every observation back toward rest duplicates the PBD material force
        and accumulates a biased visual drift.  ``reference_positions`` keeps
        the current physical deformation while constraining only this update.
        The rest-based form remains available for absolute quality reporting.
        """
        if reference_positions is None:
            reference_edge_lengths = self.rest_edge_lengths
            reference_volume_ratio = torch.ones_like(self.rest_volumes)
            reference_right_cauchy_green = torch.eye(
                3,
                dtype=corrected_positions.dtype,
                device=corrected_positions.device,
            )[None]
        else:
            if reference_positions.shape != corrected_positions.shape:
                raise ValueError("Constraint reference positions have the wrong shape")
            reference_edges = (
                reference_positions[self.edges[:, 1]]
                - reference_positions[self.edges[:, 0]]
            )
            reference_edge_lengths = torch.linalg.vector_norm(
                reference_edges, dim=1
            )
            reference_volumes = _tetra_volumes(
                reference_positions, self.tet_indices
            )
            reference_volume_ratio = reference_volumes / self.rest_volumes
            reference_points = reference_positions[self.tet_indices]
            reference_shape = torch.stack(
                (
                    reference_points[:, 1] - reference_points[:, 0],
                    reference_points[:, 2] - reference_points[:, 0],
                    reference_points[:, 3] - reference_points[:, 0],
                ),
                dim=-1,
            )
            reference_deformation_gradient = (
                reference_shape @ self.inverse_rest_shape
            )
            reference_right_cauchy_green = (
                reference_deformation_gradient.transpose(1, 2)
                @ reference_deformation_gradient
            )
        edge_vectors = (
        ...
        distance = torch.mean(
            (
                (edge_lengths - reference_edge_lengths)
        ...
        volume = torch.mean((volume_ratio - reference_volume_ratio) ** 2)
        ...
        shape = torch.mean(
            (right_cauchy_green - reference_right_cauchy_green) ** 2
        )
    ...
    def physical_quality_metrics(
        ...
        with torch.no_grad():
            ...
            weighted_mean_ratio = torch.sum(
                ratios * self.rest_volumes.abs()
            ) / self.rest_volumes.abs().sum().clamp_min(1.0e-15)
        return {
            ...
            "volume_ratio_p01": float(torch.quantile(ratios, 0.01).item()),
            "volume_ratio_p05": float(torch.quantile(ratios, 0.05).item()),
            "mean_volume_ratio": float(ratios.mean().item()),
            "volume_weighted_mean_ratio": float(weighted_mean_ratio.item()),
            "tetrahedra_below_volume_floor": int(
                torch.count_nonzero(
                    ratios < self.settings.minimum_volume_ratio
                ).item()
            ),
            "inverted_tetrahedra": int(
    ...
    def _visual_loss(
        ...
        return equal_camera_visual_loss(
            ...
            affine_color_calibration=(
                self.settings.photometric_affine_calibration
            ),
            affine_gain_minimum=self.settings.photometric_gain_minimum,
            affine_gain_maximum=self.settings.photometric_gain_maximum,
            affine_bias_limit=self.settings.photometric_bias_limit,
        )
    ...
    def solve(
        ...
        *,
        ...
        if trajectory_hold_weight < 0.0:
            raise ValueError("Trajectory hold weight must be non-negative")
        ...
        if previous_residual is None:
            ...
            temporal_reference = (
                temporal_reference * settings.previous_residual_carry
            )
        ...
        trajectory_inputs = (
            trajectory_particle_indices,
            trajectory_particle_weights,
            trajectory_confidence,
            trajectory_valid_mask,
        )
        if any(value is not None for value in trajectory_inputs) and not all(
            value is not None for value in trajectory_inputs
        ):
            raise ValueError(
                "Trajectory hold requires indices, weights, confidence and validity"
            )
        hold_indices = None
        hold_weights = None
        hold_confidence = None
        hold_valid = None
        if trajectory_particle_indices is not None:
            assert trajectory_particle_weights is not None
            assert trajectory_confidence is not None
            assert trajectory_valid_mask is not None
            hold_indices = trajectory_particle_indices.detach().to(
                device=device, dtype=torch.long
            )
            hold_weights = trajectory_particle_weights.detach().to(
                device=device, dtype=torch.float32
            )
            hold_confidence = trajectory_confidence.detach().to(
                device=device, dtype=torch.float32
            )
            hold_valid = trajectory_valid_mask.detach().to(
                device=device, dtype=torch.bool
            )
            if hold_indices.ndim != 2:
                raise ValueError("Trajectory hold indices must be [track,support]")
            if hold_weights.shape != hold_indices.shape:
                raise ValueError("Trajectory hold indices and weights disagree")
            track_count = len(hold_indices)
            if hold_confidence.shape != (track_count,):
                raise ValueError("Trajectory confidence must have one value per track")
            if hold_valid.shape != (track_count,):
                raise ValueError("Trajectory validity must have one value per track")
            weighted_slots = hold_weights > 0.0
            if bool(
                torch.any(
                    weighted_slots
                    & ((hold_indices < 0) | (hold_indices >= len(physical_positions)))
                ).item()
            ):
                raise ValueError("Trajectory hold contains an invalid particle index")
            if not bool(torch.isfinite(hold_weights).all().item()):
                raise ValueError("Trajectory hold weights contain non-finite values")
            if not bool(torch.isfinite(hold_confidence).all().item()):
                raise ValueError("Trajectory confidence contains non-finite values")
            hold_valid = (
                hold_valid
                & (hold_confidence > 0.0)
                & (hold_weights.sum(dim=1) > 0.0)
            )
            # Padded slots carry zero weight. Clamp only for safe tensor
            # indexing; they remain mathematically absent from B_j delta x.
            hold_indices = hold_indices.clamp(min=0)
        ...
        def trajectory_hold(
            ...
            if (
                hold_indices is None
                or hold_weights is None
                or hold_confidence is None
                or hold_valid is None
                or not bool(hold_valid.any().item())
            ):
                return zero, zero, zero, 0
            shifts = torch.sum(
                displacement[hold_indices]
                * hold_weights[..., None],
                dim=1,
            )[hold_valid]
            confidence = hold_confidence[hold_valid]
            squared = torch.sum(shifts.square(), dim=1)
            confidence_sum = confidence.sum().clamp_min(1.0e-12)
            mean_squared = torch.sum(confidence * squared) / confidence_sum
            loss = mean_squared / (settings.maximum_residual_m**2)
            rms = torch.sqrt(mean_squared)
            maximum = torch.linalg.vector_norm(shifts, dim=1).max()
            return loss, rms, maximum, int(torch.count_nonzero(hold_valid).item())
        ...
        for _ in range(settings.iterations):
            ...
            constraints = self._constraint_losses(
                corrected_positions,
                residual,
                reference_positions=physical_positions,
            )
            temporal = torch.mean(
                (residual - temporal_reference) ** 2
            ) / (settings.maximum_residual_m**2)
            track_hold, _, _, _ = trajectory_hold(residual)
            total = (
                ...
                + float(trajectory_hold_weight) * track_hold
            )
            total.backward()
        ...
        with torch.no_grad():
            ...
            final_track_hold, track_hold_rms, track_hold_maximum, held_tracks = (
                trajectory_hold(candidate)
            )
            final_total = (
                ...
                + float(trajectory_hold_weight) * final_track_hold
            )
        ...
        return VisualTissueResidualMappingResult(
            ...
            final_trajectory_hold_loss=float(final_track_hold.item()),
            trajectory_hold_rms_m=float(track_hold_rms.item()),
            trajectory_hold_maximum_m=float(track_hold_maximum.item()),
            trajectory_hold_track_count=held_tracks,
