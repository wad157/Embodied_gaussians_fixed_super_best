#!/usr/bin/env python3
"""Export SUPER renders and Shape-of-Motion-style EndoGaussian trajectories."""
...
def query_inputs(ground_truth):
    with np.load(str(ground_truth), allow_pickle=False) as archive:
        ...
        slot = int(np.flatnonzero(frame_indices == QUERY_FRAME)[0])
...
def export_tracks(spec, calibration, gaussians, pipe, background, alpha_threshold, render_scale, output):
    ...
    query_render = render(
        query_camera, gaussians, pipe, background, override_color=ones, stage="fine"
    )
    ...
    if int(query_valid.sum().item()) != len(point_ids):
        raise RuntimeError(
            "Query alpha/depth coverage is {}/10".format(int(query_valid.sum().item()))
        )
    intrinsic = torch.as_tensor(
        query_intrinsic, dtype=query_depth.dtype, device=query_depth.device
    )
    pixels_tensor = torch.as_tensor(
    ...
    query_centers = deformed_centers(
        gaussians, normalized_time(QUERY_FRAME, frames)
    )
    ...
    positions_world_m = np.full((len(frame_indices), len(point_ids), 3), np.nan, dtype=np.float32)
    for local_index, frame in enumerate(frame_indices):
        target = deformed_centers(gaussians, normalized_time(int(frame), frames))
        ...
        print("[track] {}/{} frame={}".format(local_index + 1, len(frame_indices), int(frame)), flush=True)
    ...
    initial_error = np.linalg.norm(uv[frame_indices == QUERY_FRAME][0] - query_pixels_full, axis=1)
    ...
    np.savez_compressed(
        ...
        query_depth_endogaussian_m=(query_depth.detach().cpu().numpy() / INTERNAL_UNITS_PER_METER).astype(np.float32),
    )
    return {
        ...
        "query_depth_endogaussian_m_min": float(query_depth.min().item() / INTERNAL_UNITS_PER_METER),
        "query_depth_endogaussian_m_max": float(query_depth.max().item() / INTERNAL_UNITS_PER_METER),
        "alignment": "none",
    }


def export_renders(dataset_key, spec, calibration, gaussians, pipe, background, render_scale, output):
    ...
    for local_index, frame in enumerate(selected):
        ...
        prediction_tensor = render(camera, gaussians, pipe, background, stage="fine")["render"]
        prediction = prediction_tensor.detach().clamp(0.0, 1.0).permute(1, 2, 0).cpu().numpy().astype(np.float32)
...
def main():
    ...
    metadata = {
        ...
        "trajectory_binding": "Shape-of-Motion-style query-anchored EndoGaussian displacement field",
        ...
        "baseline": {
            ...
            "future_behavior": "native EndoGaussian temporal deformation field extrapolation",
    ...
    (args.output_dir / "capture_summary.json").write_text(
        json.dumps({
            "schema": "endogaussian_super_capture_v1",
            "protocol": PROTOCOL,
