#!/usr/bin/env python3
"""Leakage-free SUPER inputs for the pinned EH-SurGS implementation."""
...
from utils.graphics_utils import BasicPointCloud, focal2fov, getProjectionMatrix2, getWorld2View2
...
class PackedNonInstrumentMasks(object):
    """Official baseline mask semantics: one for pixels outside the tool."""
    ...
    def get(self, frame: int) -> np.ndarray:
        slot = self.lookup.get(int(frame))
        if slot is None:
            for candidate in (int(frame) - 1, int(frame) + 1):
...
def load_training_arrays(native_root, depth_root, masks, frame, downsample):
    ...
    size = (width // int(downsample), height // int(downsample))
    if int(downsample) > 1:
    ...
    return (
        ...
        torch.from_numpy(np.ascontiguousarray(depth)),
        torch.from_numpy(np.ascontiguousarray(valid[None])),
    )


class LazyTrainingCameras(Sequence):
    """Random-access legal views without retaining all images in RAM."""
...
class SuperTrainingData(object):
    """Open only legal prefix observations; never open evaluation truth."""
    ...
    def camera_for_frame(self, frame: int, uid: int) -> Camera:
        image, depth, mask = load_training_arrays(
            self.native_root, self.depth_root, self.masks, frame, self.downsample
        )
        ...
        # Name consumed by the single audited upstream camera-intrinsic patch.
    ...
    def initial_point_cloud(self) -> BasicPointCloud:
        ...
        if not all_points:
            raise ValueError("No valid FoundationStereo points for initialization")
...
class SuperScene(object):
    def __init__(self, args, gaussians, load_iteration=None):
        ...
        if self.loaded_iter is not None:
            ...
            self.gaussians.load_ply(os.path.join(checkpoint, "point_cloud.ply"))
            self.gaussians.load_model(checkpoint)
        else:
            self.gaussians.create_from_pcd(
                data.initial_point_cloud(), self.cameras_extent, self.maxtime
            )

    def save(self, iteration, stage):
        del stage
        path = os.path.join(self.model_path, "point_cloud", "iteration_" + str(iteration))
...
def load_trained_gaussians(model_path: Path, iteration: int, sh_degree: int, hyper):
    ...
    resolved = searchForMaxIteration(str(model_root)) if iteration < 0 else int(iteration)
    checkpoint = model_root / "iteration_{}".format(resolved)
    if not checkpoint.is_dir():
        raise FileNotFoundError(checkpoint)
    gaussians = GaussianModel(sh_degree, hyper, True)
    gaussians.load_ply(str(checkpoint / "point_cloud.ply"))
    gaussians.load_model(str(checkpoint))
    mask_path = model_path / "deformation_mask.npy"
    if not mask_path.is_file():
        raise FileNotFoundError(mask_path)
    learned_mask = torch.from_numpy(np.load(str(mask_path)).astype(bool)).to("cuda:0")
    if learned_mask.shape != gaussians._deformation_table.shape:
        raise ValueError("Deformation mask and Gaussian count differ")
    return gaussians, learned_mask, resolved


def deformation_selection(gaussians, learned_mask):
    selected = gaussians._deformation_table.clone()
    selected[: min(4000, len(selected))] = False
    return selected | learned_mask


def deformed_centers(gaussians, learned_mask, time):
    means = gaussians.get_xyz
    selected = deformation_selection(gaussians, learned_mask)
    result = means.clone()
    if bool(selected.any()):
        deformation = gaussians.gaussian_deformation(
            torch.tensor(float(time), dtype=means.dtype, device=means.device),
            selected,
            num_gaussians=int(selected.sum().item()),
            ch_num=gaussians.args.ch_num,
            basis_num=gaussians.args.curve_num,
        )
        deformed, _, _, _ = gaussians.apply_deformations(
            means[selected].clone(),
            gaussians._scaling[selected].clone(),
            gaussians._rotation[selected].clone(),
            gaussians._opacity[selected].clone(),
            selected,
            deformation,
            float(time),
            True,
