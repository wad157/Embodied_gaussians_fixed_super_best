...
Gaussian 的 RGB、opacity、静止四元数和三轴尺度沿用已重建外观；尺度约
`0.175–0.700 mm`，各向异性中位数约 `1.91`，并不是用圆形物理粒子代替表面外观。
...
| Gaussian RGB/opacity/scale | 固定外观数组 | 重建资产值 | residual 模式不更新 |
...
九个物理节点；静止 offset-frame 旋转项暂未进入这一阶平移 Jacobian。residual 渲染只
打包组织 Gaussian；器械区域由 2D mask 抑制。Gaussian quaternion、scale、RGB 和 opacity
均 `detach` 并保持固定，当前可微图像变量只有由节点 residual 生成的 Gaussian mean。
...
节点写入临时状态，用正式视觉顶点嵌入和 Gaussian 蒙皮重新计算 mean/quaternion，再渲染
