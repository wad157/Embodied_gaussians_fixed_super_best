...
OUTPUT_ROOT="${1:-${ROOT_DIR}/outputs/super_protocol_corrected_full_evaluation_20260823_v1}"
...
run_case() {
    ...
    if [[ "${method}" == "residual_only" ]]; then
        ...
        extra_args+=(
            ...
            --stiffness-maximum-log-step 0.30
    ...
    CUDA_VISIBLE_DEVICES="${gpu_id}" \
        TORCH_EXTENSIONS_DIR="/Media_HDD/jwshan/tmp/torch_extensions_cross_frame_gpu${gpu_id}" \
...
run_lane_zero() {
    run_pair extreme_soft pure_pbd 0 0.05 0.002
    run_pair extreme_soft residual_only 0 0.05 0.002
    run_pair extreme_soft residual_online_robust 0 0.05 0.002
    run_pair moderate pure_pbd 0 0.20 0.004
    run_pair moderate residual_only 0 0.20 0.004
}

run_lane_one() {
    run_pair extreme_hard pure_pbd 1 3.20 0.030
    run_pair extreme_hard residual_only 1 3.20 0.030
    run_pair extreme_hard residual_online_robust 1 3.20 0.030
...
set +e
"${ENV_PREFIX}/bin/python" \
    scripts/validate_super_online_dominance.py \
    --root "${OUTPUT_ROOT}" > "${OUTPUT_ROOT}/dominance.log" 2>&1
dominance_status=$?
set -e
if [[ "${dominance_status}" -ne 0 ]]; then
    printf 'dominance_status=%s\n' "${dominance_status}" \
        > "${OUTPUT_ROOT}/DOMINANCE_FAILED"
    exit 4
fi
