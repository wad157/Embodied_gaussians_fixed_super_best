...
CAMERA_PATCH="$ROOT_DIR/baselines/eh_surgs_super/patches/train_camera_intrinsics.patch"
CAMERA_PATCH_SHA256="f75e5d151e7f885024e6c75ccc3a1457ebc86389d8277d433cae58175bc310d8"

if [[ ! -x "$SOURCE_ENV/bin/python" ]]; then
    echo "ERROR: missing Python 3.7 source environment: $SOURCE_ENV" >&2
    exit 1
fi
if [[ ! -x "$CUDA_TOOLKIT/bin/nvcc" ]]; then
    echo "ERROR: missing consistent CUDA 11.8 toolkit: $CUDA_TOOLKIT" >&2
    exit 1
fi
if [[ ! -d "$UPSTREAM_ROOT/.git" ]]; then
    if [[ -e "$UPSTREAM_ROOT" ]]; then
        echo "ERROR: upstream path exists but is not a Git checkout: $UPSTREAM_ROOT" >&2
        exit 1
    fi
...
if [[ "$(git -C "$UPSTREAM_ROOT" rev-parse HEAD)" != "$UPSTREAM_COMMIT" ]]; then
    echo "ERROR: EH-SurGS checkout is not pinned to $UPSTREAM_COMMIT" >&2
    exit 1
fi
...
if [[ "$actual_patch_sha256" != "$CAMERA_PATCH_SHA256" ]]; then
    echo "ERROR: camera compatibility patch differs: $actual_patch_sha256" >&2
    exit 1
fi
...
BUILD_ROOT="$(mktemp -d /tmp/eh_surgs_super_extensions.XXXXXX)"
...
CUDA_VISIBLE_DEVICES="${EH_SURGS_GPU_ID:-0}" \
    bash "$ROOT_DIR/scripts/run_eh_surgs_super_python.sh" -c \
    'import torch; from simple_knn._C import distCUDA2; import diff_gaussian_rasterization as d; x=torch.randn(5000,3,device="cuda"); assert distCUDA2(x).shape==(5000,); print(torch.__version__, torch.version.cuda, torch.cuda.get_device_name(0)); print(d.__file__)'
