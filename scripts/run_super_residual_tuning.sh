...
OUTPUT_ROOT="${1:-${ROOT_DIR}/outputs/super_residual_tuning_phase4_20260821}"
GROUND_TRUTH="${ROOT_DIR}/data/super/evaluation_v1/manual_tissue_tracks_10/ground_truth_2d3d_v1.npz"

if [[ -e "${OUTPUT_ROOT}" ]]; then
    printf 'Refusing reused tuning output: %s\n' "${OUTPUT_ROOT}" >&2
    exit 2
fi
if [[ ! -x "${ENV_PREFIX}/bin/python" || ! -f "${GROUND_TRUTH}" ]]; then
    printf 'Missing environment or ground truth.\n' >&2
    exit 2
fi
mkdir -p "${OUTPUT_ROOT}"
cd "${ROOT_DIR}"
...
(
    ...
    run_case residual_step_0p05mm 0 residual 5e-6 5e-5
) > "${OUTPUT_ROOT}/gpu0.log" 2>&1 &
gpu0_pid=$!
(
    run_case residual_step_0p10mm 1 residual 1e-5 1e-4
    run_case residual_step_0p20mm 1 residual 2e-5 2e-4
