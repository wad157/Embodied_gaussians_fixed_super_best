...
def read_baseline_m(calibration: dict) -> tuple[float, str]:
    """Read a metric stereo baseline from current or legacy SUPER calibration."""
    if "baseline_m" in calibration:
        return float(calibration["baseline_m"]), "baseline_m"

    # Legacy grasp1/grasp3 calibration stores the OpenCV stereo translation T
    # in millimetres but omits the convenience field added for grasp5.
    if "T" in calibration:
        translation_mm = np.asarray(calibration["T"], dtype=np.float64)
        if translation_mm.shape == (3,) and np.all(np.isfinite(translation_mm)):
            baseline_m = float(np.linalg.norm(translation_mm) / 1000.0)
            if baseline_m > 0.0:
                return baseline_m, "norm(T)_mm_to_m"

    raise KeyError(
        "Calibration must contain baseline_m or a finite three-vector T in mm"
    )
...
def main() -> None:
    ...
    fx = float(K_left[0, 0])
    baseline, baseline_source = read_baseline_m(calibration)
    ...
    for frame in frames:
        ...
        if insertion == 0:
            before = after = 0
            alpha = 0.0
            timestamp_sampling_mode = "nearest_boundary_zero_order_hold"
        elif insertion == len(right_timestamps):
            before = after = len(right_timestamps) - 1
            alpha = 0.0
            timestamp_sampling_mode = "nearest_boundary_zero_order_hold"
        else:
            before = insertion - 1
            ...
            timestamp_sampling_mode = "bracketed_linear_disparity_interpolation"
        ...
        report = {
            ...
            "timestamp_sampling_mode": timestamp_sampling_mode,
    ...
    summary = {
        ...
        "timestamp_method": (
            "linearly interpolate disparity between right frames bracketing each "
            "left timestamp; use nearest boundary frame with zero-order hold "
            "when no two-sided bracket exists"
        ),
        "confidence_method": (
