#!/usr/bin/env python3
"""CPU gate for exact visual-face Gaussian deformation covariance."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--warp-cache-dir",
        type=Path,
        default=Path("/tmp/warp-super-deformation-covariance"),
    )
    args = parser.parse_args()
    wp.config.kernel_cache_dir = str(args.warp_cache_dir)
    wp.init()

    body = SoftBody.from_npz(args.asset, name="super_tissue")
    ...
    mapper = TetrahedralGaussianVisualResidualMapper.from_visual_face_centroid_bindings(
        ...
        soft_gaussian_ids=gaussian_model.soft_gaussian_ids,
        ...
        rest_gaussian_quats=gaussian_model.quats,
        rest_gaussian_scales=gaussian_model.scales,
    )
    ...
    runtime_rest_covariances = covariance(state.quats, state.scales)
    rest_mean_error = torch.linalg.vector_norm(
        state.means - rest_means, dim=1
    ).max()
    ...
    runtime_covariances = covariance(state.quats, state.scales)
    deformed_mean_error = torch.linalg.vector_norm(
        state.means - mapped_means, dim=1
    ).max()
    ...
    scale_change = torch.linalg.vector_norm(
        state.scales - gaussian_model.scales, dim=1
    )
    report = {
        "stage": "super_visual_face_deformation_covariance_gate",
    ...
    report["gates"] = {
        ...
        "rest_geometry_matches_runtime": bool(
            rest_mean_error < 2.0e-7 and rest_covariance_error < 2.0e-10
        ),
        "deformed_geometry_matches_runtime": bool(
            deformed_mean_error < 2.0e-7
            and deformed_covariance_error < 2.0e-10
        ),
        "triangle_deformation_changes_scales": bool(scale_change.mean() > 1.0e-6),
