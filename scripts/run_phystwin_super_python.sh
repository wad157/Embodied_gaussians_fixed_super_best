...
if [[ ! -x "${ENV_ROOT}/bin/python" ]]; then
    echo "Missing PhysTwin environment: ${ENV_ROOT}" >&2
    exit 1
fi
if [[ ! -d "${UPSTREAM_ROOT}/.git" ]]; then
    echo "Missing pinned PhysTwin checkout: ${UPSTREAM_ROOT}" >&2
    exit 1
fi
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/phystwin_super_mpl}"
export PYTHONNOUSERSITE=1
export PYTHONPATH="${ROOT_DIR}/baselines/phystwin_super:${UPSTREAM_ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
