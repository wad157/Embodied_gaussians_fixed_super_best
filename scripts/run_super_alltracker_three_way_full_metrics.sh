...
OUTPUT_ROOT="${1:-${ROOT_DIR}/outputs/super_alltracker_three_way_full_metrics_20260829_v1}"
...
sha256sum \
    ...
    scripts/run_super_alltracker_three_way_full_metrics.sh \
    "${BINDINGS}" \
    "${OBSERVATIONS}" \
...
printf 'Formal AllTracker three-way full-metric evaluation complete: %s\n' \
    "${OUTPUT_ROOT}"
