...
EG_PY="${ROOT_DIR}/scripts/run_endogaussian_super_python.sh"
EVAL_ENV="${EVAL_ENV_PREFIX:-/Media_HDD/jwshan/conda_envs/eg_codex}"
EVAL_PY="${EVAL_ENV}/bin/python"
GPU_ID="${ENDOGAUSSIAN_GPU_ID:-0}"
CONFIG="${ENDOGAUSSIAN_CONFIG:-${ROOT_DIR}/baselines/endogaussian_super/configs/super_unified.py}"

if [[ $# -ne 4 ]]; then
    echo "Usage: bash scripts/run_endogaussian_super_baseline_once.sh <grasp5|grasp3|grasp1> <repeat_01|repeat_02|repeat_03> <seed> <OUTPUT_ROOT>" >&2
    exit 2
fi
DATASET_KEY="$1"
REPEAT_ID="$2"
...
if [[ ! "${SEED}" =~ ^[0-9]+$ ]] || [[ ! "${GPU_ID}" =~ ^[0-9]+$ ]]; then
    echo "Seed and ENDOGAUSSIAN_GPU_ID must be non-negative integers" >&2
    exit 2
fi
...
PORT="${ENDOGAUSSIAN_PORT:-$((6500 + SEED * 10 + DATASET_INDEX))}"
...
printf '%s\n' \
    ...
    "seed=${SEED}" \
    "gpu=${GPU_ID}" \
    'method=EndoGaussian_super_v2_noninstrument_mask' \
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${EVAL_PY}" \
    ...
    --dataset-key "${DATASET_KEY}" --device cuda:0 \
    > "${LOG}" 2>&1

printf 'status=auditing\n' > "${OUTPUT_ROOT}/status.txt"
"${EVAL_PY}" "${ROOT_DIR}/scripts/audit_endogaussian_super_protocol.py" \
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" bash "${EG_PY}" \
    "${ROOT_DIR}/baselines/endogaussian_super/train_super.py" \
    --dataset-key "${DATASET_KEY}" --seed "${SEED}" \
    --source_path "${NATIVE}" --model_path "${MODEL_PATH}" \
    --configs "${CONFIG}" --port "${PORT}" \
    >> "${LOG}" 2>&1

printf 'status=exporting\n' > "${OUTPUT_ROOT}/status.txt"
CUDA_VISIBLE_DEVICES="${GPU_ID}" bash "${EG_PY}" \
    "${ROOT_DIR}/baselines/endogaussian_super/export_super.py" \
