...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Frozen table frame; defaults to source-dir/table_frame.json when present.",
    )
    parser.add_argument(
        "--camera-manifest",
        type=Path,
        default=None,
        help="Frozen table-space cameras; defaults to source-dir/cameras_table.json when present.",
    )
...
def retain_largest_contact_component(
    points: np.ndarray, diameter: float
    ...
    keep = labels == largest
    return points[keep], {
        "contact_threshold_mm": float((diameter + 1.0e-7) * 1000.0),
        "components_before_filter": int(count),
        "component_sizes_before_filter": sorted(map(int, sizes), reverse=True),
        "removed_particle_count": int(np.count_nonzero(~keep)),
        "kept_particle_count": int(np.count_nonzero(keep)),
        "largest_component_fraction": float(np.mean(keep)),
        "components_after_filter": 1,
    }
...
def main() -> None:
    ...
    local_table_frame = args.source_dir / "table_frame.json"
    local_cameras = args.source_dir / "cameras_table.json"
    table_frame = args.table_frame or (
        local_table_frame
        if local_table_frame.is_file()
        else REPO / "data/super/table_frame.json"
    )
    camera_manifest = args.camera_manifest or (
        local_cameras
        if local_cameras.is_file()
        else REPO / "data/super/grasp5_offline_demo/cameras.json"
    )
    frozen_paths = {"table_frame": table_frame, "cameras": camera_manifest}
    for path in frozen_paths.values():
        if not path.is_file():
            raise FileNotFoundError(path)
    frozen_hashes = {name: sha256(path) for name, path in frozen_paths.items()}
    using_historical_grasp5_defaults = (
        table_frame.resolve() == (REPO / "data/super/table_frame.json").resolve()
        and camera_manifest.resolve()
        == (REPO / "data/super/grasp5_offline_demo/cameras.json").resolve()
    )
    if using_historical_grasp5_defaults and frozen_hashes != EXPECTED_SHA256:
    ...
    candidate_world, contact_connectivity = retain_largest_contact_component(
        candidate_world, 2.0 * radius
    )
    ...
    shutil.copy2(table_frame, args.output_dir / "table_frame.json")
    shutil.copy2(camera_manifest, args.output_dir / "cameras_table.json")
    ...
    metadata = {
        **source_metadata,
        ...
        "source_table_dir": str(args.source_dir.resolve()),
        "source_table_tissue_sha256": sha256(source_paths["tissue.json"]),
        ...
        "coordinate_guard": {
            **source_metadata.get("coordinate_guard", {}),
            "table_frame": str(table_frame.resolve()),
            "table_frame_sha256": frozen_hashes["table_frame"],
            "camera_manifest": str(camera_manifest.resolve()),
