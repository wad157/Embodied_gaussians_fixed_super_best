#!/usr/bin/env python3
"""Compare long-horizon and cross-signed online-stiffness ablations."""
...
def read_stiffness(path: Path) -> dict[str, object]:
    status = Counter()
    variants = Counter()
    pending_evidence = 0
    latest: dict = {}
    with path.open(encoding="utf-8") as stream:
        for line in stream:
            ...
            if event.get("event") == "visual_update":
                pending_evidence += int(
                    bool(material.get("evidence_accumulated_while_pending"))
                )
            if event.get("event") == "stiffness_validation":
                validation = material.get(
    ...
    return {
        ...
        "pending_evidence_visual_updates": pending_evidence,
        "final_distance": [
...
def main() -> None:
    ...
    for name in ("h10_bidirectional_8", "h10_cross_signed_12"):
    ...
    diagnostics = {
        name: read_stiffness(
            root / name / "stiffness_diagnostics/events.jsonl"
        )
        for name in ("h10_bidirectional_8", "h10_cross_signed_12")
    }
    output = {
        "schema": "super_stiffness_future_algorithm_ablation_v1",
        "metrics": metrics,
        "effects": effects,
        "diagnostics": diagnostics,
    }
    (root / "ALGORITHM_ABLATION.json").write_text(
        json.dumps(output, indent=2) + "\n", encoding="utf-8"
    )
    lines = [
        "# Online stiffness future algorithm ablation",
        "",
    ...
    (root / "ALGORITHM_ABLATION.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
